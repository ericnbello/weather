...
# @ensure_csrf_cookie
def index(request):
    return render(request, 'templates/components/form.html')
...
#         if units == 'imperial':
#             r_2 = httpx.get('https://api.openweathermap.org/data/2.5/onecall?lat={0}&lon={1}&exclude=minutely&units={2}&appid={3}'.format(lat, lon, units, api_key))
#             degree_unit = '˚F'
#             speed_unit = 'mph'
#             pressure_unit  = 'mb'
#         else:
...
#         current_utc_time = datetime.fromtimestamp(current_unix_timestamp, timezone.utc)
#         city_timezone = pytz.timezone(weather_data_2["timezone"])
#         # local_time = current_utc_time.astimezone(pytz.timezone(city_timezone))
#         local_time = current_utc_time.astimezone(city_timezone)
#         current_date = local_time.strftime("%A %b %d, %Y").lstrip("0").replace(" 0", " ")
#         current_time = local_time.strftime("%I:%M %p").lstrip("0").replace(" 0", " ")
